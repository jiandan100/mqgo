...
import (
	...
	"app/src/confopt/log"
	"app/src/confopt/mqueue/aliutils"
)
...
func initMqGroup() map[string]MQ {
	var groups = make(map[string]MQ)
	if mqCfg == nil {
		log.Error("mq config setting error")
	}
	for g, e := range mqCfg.Queues {
		group, err := New(e)
		if err != nil {
			log.Error("创建消息队列连接错误：" + err.Error())
			return nil
		}
		groups[g] = group
		log.Info(fmt.Sprintf("%s MqGroup Opened", g))
	}
	return groups
}

func Init(mqCfgFile string) {
	buf, err := ioutil.ReadFile(mqCfgFile)
	if err != nil {
		log.Warn(mqCfgFile + "文件读取失败")
	}
	err = yaml.Unmarshal(buf, &mqCfg)
	if err != nil {
		log.Warn(mqCfgFile + "解析失败")
	}
	MqGroup = initMqGroup()
}
...
func Use(mqName string) MQ {
	...
	if g, ok := MqGroup[mqName]; ok {
		...
		log.Error(mqName + " - mq does not exist.")
	}
	return nil
}
...
func Close() {
	for n, mq := range MqGroup {
		mq.Close()
		log.Info(fmt.Sprintf("%s mq Closed", n))
	}
}
...
func (mq *mq) initializeConsumersWorker(consumer *consumer, worker *worker) error {
	...
	deliveries, err := channel.Consume(consumer.queue, consumer.name, autoAck, exclusive, noLocal, noWait, args)
	if err != nil {
		return err
	}
